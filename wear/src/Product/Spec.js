...
const Spec = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:8700/api/Product/${id}`).then((response) => {
    ...
    const CheckServiceability = async () => {
        axios.get('http://localhost:8700/api/Pincode').then((response)=>{
        let pins = response.data
        
        if (pins.includes(parseInt(pin))) {
            setService(true)
            toast.success('🚀 Hurray We Deliver Here!', {
            ...
            <section className="text-gray-600 body-font overflow-hidden">
                ...
                <div className="container px-5 py-20 mx-auto">
                    <div className="lg:w-4/5 mx-auto flex flex-wrap">
                        <img alt="ecommerce" className="lg:w-1/2 w-full lg:h-auto px-20  object-cover object-top rounded" src={product.image} />
                        <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                            <h2 className="text-sm title-font text-gray-500 tracking-widest">{product.category}</h2>
                            <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{product.brand}</h1>
